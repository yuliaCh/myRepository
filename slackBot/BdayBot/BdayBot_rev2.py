...
to employees in Slack messenger based on provided google spreadsheet containing employees' dates of birth.
 In order for the program to run successfully, config argument should be supplied. 
 The argument is not positional, but it is required.''')
    parser.add_argument('-c', '--config', required=True, type = argparse.FileType(), help='''config file,
     containing slack token, path to json file with google account credentials and some other data;
      this argument is not positional,
      but it is required.''')
...
Напиши @obogdanova, чтобы узнать подробности!""",
                                      as_user="true")
                logger.info("Birthday notification has been sent to {}".format(user["name"]))
            except KeyError:
...
def main():
    ...
    logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(name)s %(levelname)s: %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S', filename=config["log_file"])
