...
function getStarted() {
	if(elementsBlock.children[1].value == "Старт!") {
		rounds++;
		selectNumber();
		changeHeading();
		changeFieldset(1);
...
function changeHeading() {
	document.getElementById("statusField").children[0].innerHTML = "Компьютер загадал число от 0 до 100. Игра состоит из 10 раундов.";
}
...
function changeButton() {
	elementsBlock.children[1].value = "Заново";
}
...
function changeRounds() {
	elementsBlock.children[0].innerHTML = "Раунд " + rounds;
...
function changeHeading1() {
	document.getElementById("statusField").children[0].innerHTML = "Введите целое число";
...
function changeHeading2(j) {
	...
	if (getPlayerNumber(j) < 0 || getPlayerNumber(j) > 100) {
		document.getElementById("statusField").children[0].innerHTML = "Число должно быть между 0 и 100 включительно";
		document.getElementById("statusField").children[0].style.color ="red";
	}
...
function compareNumbers(j) {
	...
	if(getPlayerNumber(j) == number) {
		...
		document.getElementById("congradsBox").children[2].innerHTML = "Игрок "+b+" указал верное число!"
		...
		if(j==1){
			...
			if(rounds==11){
				...
				document.getElementsByClassName("msg")[2].children[2].innerHTML = "У вас больше не осталось попыток."+"<br>"+" Правильный ответ: "+number+".";
				document.getElementsByTagName("FIELDSET")[j].children[1].blur();
				return;
			}
			else {
		...
		document.getElementById("statusField").children[0].innerHTML = a +" меньше загаданного числа";
		...
		document.getElementById("statusField").children[0].innerHTML = a +" больше загаданного числа";
		document.getElementById("statusField").children[0].style.color ="black";
		changeFieldset(j);
	}
} 
